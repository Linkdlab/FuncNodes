...
3. Install required modules if not present: `funcnodes-files`, `funcnodes-pandas`, `funcnodes-plotly`.
4. Import the workflow:
   - Open **Nodespace → Import**, choose `titanic.fnw` (bundled with this example).
5. Drop in the sample Titanic CSV (any standard copy works) via the file upload node, or point to a local path under the worker `files/` directory.
