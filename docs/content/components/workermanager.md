...
Workers with `autostart: "always"` are started whenever a refresh finds them
inactive. Workers with `autostart: "unless-stopped"` are also started
automatically, but a manual stop suppresses further autostarts until the worker
is manually started again or the manager session restarts.
