...
2. Choose a **workflow-local config directory** (default: next to the file).
3. Ensure a **dedicated worker** exists (and start it if needed).
4. Import the `.fnw` content into that worker.
5. Start a small UI server (React Flow UI) connected to that worker.
6. Optionally open your browser.
...
1. **Write a launcher script** into your FuncNodes config directory:
   - Windows: `.../scripts/fnw_open.cmd`
   - Linux/macOS: `.../scripts/fnw_open.sh`
2. **Register** this launcher as the default opener for `.fnw` files (best-effort, per user).
...
!!! note "Windows default-app behavior"
...
- [CLI Reference](../api/cli.md) — all FuncNodes commands
- [Workers](../components/worker.md) — how workers execute workflows
- [Configuration](../components/config.md) — global and worker configuration
