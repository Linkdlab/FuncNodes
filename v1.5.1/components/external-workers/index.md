...
1. Apply changes to internal state/resources.
1. If the *library surface* changed (shelf contents or UI options), emit a nodes update.
...
1. **Local scripts (development/experiments)** A Worker can scan a local scripts directory and discover subclasses placed there.
1. Default location: `<worker_data_path>/local_scripts/`
1. Any `*.py` file in that folder (recursively) can define subclasses of `FuncNodesExternalWorker`.
1. Discovered classes become available to instantiate as external worker instances.
1. **Module-provided external workers (packaged distribution)** A FuncNodes module can advertise external worker classes via its `funcnodes.module` entry points under the key `external_worker`.
