...
class PDReadCSV(Node):
    ...
    def try_read_csv(self, value, sep, decimal) -> pd.DataFrame:
        try:
            return pd.read_csv(value, index_col=None, sep=sep, decimal=decimal)
        except (FileNotFoundError, OSError):
            from io import StringIO

            # maybe the string is not a path but a datastring
            return self.try_read_csv(StringIO(value), sep=sep, decimal=decimal)

    async def on_trigger(self):
        sep = self.sep.value
        dec = self.decimal.value
        self.output.value = self.try_read_csv(self.input.value, sep=sep, decimal=dec)
        return True
